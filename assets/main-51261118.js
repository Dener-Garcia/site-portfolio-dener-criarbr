import{A as f,C as p,s as y}from"./scale-text-9bbb82df.js";import{S as g}from"./show-menu-7b28359c.js";import{R as C}from"./animation-reveral-6b326b36.js";class w extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=`
    ...
    `}}const u=()=>{const i=document.querySelector("form"),t=document.querySelector("#fieldName"),e=document.querySelector("#fieldMail"),s=document.querySelector("#fieldMessage"),r=document.querySelector("form button");i.addEventListener("submit",o=>{if(o.preventDefault(),a(t),a(e),a(s),!a(t))return;if(!d(e.value)){h(e,"Ops! E-mail incorreto. Tente algo como: seu_email@email.com.br");return}if(!a(s))return;const n={name:t.value,mail:e.value,message:s.value};fetch("https://apisendmail.criarbr.com/enviar-email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)}).then(c=>c.json()).then(c=>{c.message?(k([t,e,s]),console.info("Resposta do server:",c.message)):(l("Falha no envio, tente novamente","ph","ph-warning-diamond","inputInvalidJS"),console.error("Resposta do server:",c.error))}).catch(c=>{l("Falha no envio, tente novamente","ph","ph-warning-diamond","inputInvalidJS"),console.error("Erro ao enviar dados do formulário:",c)})});function a(o){return o.value===""?(h(o,"Ops! Você esqueceu de preencher esse campo"),!1):(h(o,""),o.classList.remove("inputInvalidJS"),o.classList.add("inputValidJS"),!0)}function d(o){return!!new RegExp(/^[a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z]{2,}$/).test(o)}function h(o,n){const m=o.parentElement.querySelector("legend");m.textContent=n,o.classList.remove("inputValidJS"),o.classList.add("inputInvalidJS")}function k(o){setTimeout(()=>{o.forEach(n=>{n.classList.remove("inputValidJS"),n.value=""}),l("Mensagem enviada","ph","ph-check-fat","inputValidJS")},1500)}function l(o,n,c,m){r.textContent=o,r.classList.add(m);const F=document.createElement("i");F.classList.add(n),F.classList.add(c),r.appendChild(F)}},L=()=>{const i=document.querySelectorAll(".certificate-box");i.forEach(t=>{t.addEventListener("mouseenter",()=>{const e=t.querySelector("details");e.hasAttribute("open")?e.removeAttribute("open"):e.setAttribute("open","")})}),i.forEach(t=>{t.addEventListener("mouseEnter",()=>{const e=t.querySelector("details");e.hasAttribute("open")?e.removeAttribute("open"):e.setAttribute("open","")})})},x=async()=>{try{const t=await(await fetch("/card-projects.json")).json(),e=document.querySelector(".grid-projects");t.contentCard.map(s=>{const r=s,a=document.createElement("div");a.classList.add("card-project");const d=document.createElement("img");d.src=r.imgLink,d.alt=r["alt-img"],a.appendChild(d);const h=document.createElement("h3");h.textContent=r.title,a.appendChild(h);const k=document.createElement("p");k.textContent=r.description,a.appendChild(k);const l=document.createElement("a");l.classList.add("btn-primary"),l.textContent="Ver Projeto",l.href=r.linkPage,l.setAttribute("data",r.name),a.appendChild(l),e.appendChild(a)})}catch(i){console.error("Erro na requisição:",i)}},v=()=>{document.querySelector(".grid-projects").addEventListener("pointerover",()=>{document.querySelectorAll(".card-project a").forEach(e=>{let s=e.getAttribute("data");e.addEventListener("click",()=>{localStorage.setItem("name-link",s)})})})},M=(i,t)=>{const e=i;document.addEventListener("scroll",()=>{console.log("chamo funcao"),e.forEach(s=>{let r=window.innerHeight;s.getBoundingClientRect().top<r-t?s.classList.remove("reveral-left"):s.classList.add("reveral-left")})})},E=(i,t)=>{const e=i;document.addEventListener("scroll",()=>{e.forEach(s=>{let r=window.innerHeight;s.getBoundingClientRect().top<r-t?s.classList.remove("reveral-right"):s.classList.add("reveral-right")})})};window.addEventListener("load",()=>{window.customElements.define("my-svg",w)});f();p();y();g();u();L();x();v();const b=document.querySelectorAll(".certificate-box");C(b,-100);const S=document.querySelectorAll(".left");M(S,50);const q=document.querySelectorAll(".right");E(q,50);
