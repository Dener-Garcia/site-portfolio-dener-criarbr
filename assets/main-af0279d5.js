import{A as F,C as p,s as y,S as g}from"./show-menu-5f152794.js";class C extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=`
    ...
    `}}const u=()=>{const a=document.querySelector("form"),s=document.querySelector("#fieldName"),e=document.querySelector("#fieldMail"),t=document.querySelector("#fieldMessage"),r=document.querySelector("form button");a.addEventListener("submit",o=>{if(o.preventDefault(),i(s),i(e),i(t),!i(s))return;if(!d(e.value)){h(e,"Ops! E-mail incorreto. Tente algo como: seu_email@email.com.br");return}if(!i(t))return;const n={name:s.value,mail:e.value,message:t.value};fetch("https://apisendmail.criarbr.com/enviar-email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)}).then(c=>c.json()).then(c=>{c.message?(m([s,e,t]),console.info("Resposta do server:",c.message)):(l("Falha no envio, tente novamente","ph","ph-warning-diamond","inputInvalidJS"),console.error("Resposta do server:",c.error))}).catch(c=>{l("Falha no envio, tente novamente","ph","ph-warning-diamond","inputInvalidJS"),console.error("Erro ao enviar dados do formulário:",c)})});function i(o){return o.value===""?(h(o,"Ops! Você esqueceu de preencher esse campo"),!1):(h(o,""),o.classList.remove("inputInvalidJS"),o.classList.add("inputValidJS"),!0)}function d(o){return!!new RegExp(/^[a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z]{2,}$/).test(o)}function h(o,n){const k=o.parentElement.querySelector("legend");k.textContent=n,o.classList.remove("inputValidJS"),o.classList.add("inputInvalidJS")}function m(o){setTimeout(()=>{o.forEach(n=>{n.classList.remove("inputValidJS"),n.value=""}),l("Mensagem enviada","ph","ph-check-fat","inputValidJS")},1500)}function l(o,n,c,k){r.textContent=o,r.classList.add(k);const f=document.createElement("i");f.classList.add(n),f.classList.add(c),r.appendChild(f)}},w=()=>{const a=document.querySelectorAll(".certificate-box");a.forEach(s=>{s.addEventListener("mouseenter",()=>{const e=s.querySelector("details");e.hasAttribute("open")?e.removeAttribute("open"):e.setAttribute("open","")})}),a.forEach(s=>{s.addEventListener("mouseEnter",()=>{const e=s.querySelector("details");e.hasAttribute("open")?e.removeAttribute("open"):e.setAttribute("open","")})})},L=async()=>{try{const s=await(await fetch("/card-projects.json")).json(),e=document.querySelector(".grid-projects");s.contentCard.map(t=>{const r=t,i=document.createElement("div");i.classList.add("card-project");const d=document.createElement("img");d.src=r.imgLink,d.alt=r["alt-img"],i.appendChild(d);const h=document.createElement("h3");h.textContent=r.title,i.appendChild(h);const m=document.createElement("p");m.textContent=r.description,i.appendChild(m);const l=document.createElement("a");l.classList.add("btn-primary"),l.textContent="Ver Projeto",l.href=r.linkPage,l.setAttribute("data",r.name),i.appendChild(l),e.appendChild(i)})}catch(a){console.error("Erro na requisição:",a)}},x=()=>{document.querySelector(".grid-projects").addEventListener("pointerover",()=>{document.querySelectorAll(".card-project a").forEach(e=>{let t=e.getAttribute("data");e.addEventListener("click",()=>{localStorage.setItem("name-link",t)})})})},v=(a,s)=>{const e=a;document.addEventListener("scroll",()=>{e.forEach(t=>{let r=window.innerHeight;t.getBoundingClientRect().top<r-s?t.classList.remove("reveral-up"):t.classList.add("reveral-up")})})},M=(a,s)=>{const e=a;document.addEventListener("scroll",()=>{console.log("chamo funcao"),e.forEach(t=>{let r=window.innerHeight;t.getBoundingClientRect().top<r-s?t.classList.remove("reveral-left"):t.classList.add("reveral-left")})})},E=(a,s)=>{const e=a;document.addEventListener("scroll",()=>{e.forEach(t=>{let r=window.innerHeight;t.getBoundingClientRect().top<r-s?t.classList.remove("reveral-right"):t.classList.add("reveral-right")})})};window.addEventListener("load",()=>{window.customElements.define("my-svg",C)});F();p();y();g();u();w();L();x();const b=document.querySelectorAll(".certificate-box");v(b,-100);const S=document.querySelectorAll(".left");M(S,50);const R=document.querySelectorAll(".right");E(R,50);
